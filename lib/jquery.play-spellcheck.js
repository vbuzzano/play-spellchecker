/*
 * jquery.play-spellchecker.js - a simple Play Module for Spell Checker
 * 
 * javascript based on 
 * jquery.spellchecker.js by Richard Willis
 * 
 *  Copyright (c) 2011 Vincent Buzzano <vincent.buzzano@gmail.com>
 *  
 * MIT license  : http://www.opensource.org/licenses/mit-license.php
 * Project      : TODO
 * Contact      : vincent.buzzano@gmail.com
 */
...
(function($){
	...
	var SpellChecker = function(domObj, options) {
		...
		this.options = $.extend({
			...
			engine: 'pspell',				// internal, pspell or google
	...
	SpellChecker.prototype = {
		...
		highlightWords : function(json, callback) {
			if (!json.length) { (callback) && callback(this.$domObj, true); return; }
			...
			$.each(json, function(key, badWord){
				self.badwordlist[badWord.term] = badWord;
				
				// TODO faire le call back ci plutot que
				// dans la boucle en dessous
				html = html.replace(
					new RegExp('([^a-zA-Z\\u00A1-\\uFFFF])('+badWord.term+')([^a-zA-Z\\u00A1-\\uFFFF])', 'g'),
		...
		buildBadwordsBox : function(json, callback){
			...
			self.badwordlist = [];
			$.each(json, function(key, badword) {
				if ($.inArray(badword.term, words) === -1) {
					self.badwordlist[badword.term] = badword;
					self.elements.highlightWords.push(
						$('<span class="spellcheck-word-highlight">'+badword.term+'</span>')
						.click(function(){ self.suggest(this); })
						.appendTo(self.elements.$badwords)
						.after('<span class="spellcheck-sep">,</span> ')
					);
					words.push(badword.term);
				}
			});
		...
		suggest : function(word){
			...
			var badword = self.badwordlist[this.$curWord.html()];
			...
			self.buildSuggestBox(badword.list, offset);
		},
